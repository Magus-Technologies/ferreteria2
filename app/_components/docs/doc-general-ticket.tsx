      ...
      >
        ...
        >
          ...
          {total_descuento !== undefined && total_descuento > 0 && (
            <View style={{ ...styles_ticket.total, borderBottom: '1px solid #000' }}>
              <Text style={{ fontSize: 7, fontWeight: 'bold' }}>TOTAL DESCUENTO</Text>
              <Text style={{ fontSize: 7 }}>{Number(total_descuento).toFixed(2)}</Text>
            </View>
          )}
          ...
          {op_gravada !== undefined && (
            <View style={{ ...styles_ticket.total, borderBottom: '1px solid #000' }}>
              <Text style={{ fontSize: 7, fontWeight: 'bold' }}>OP.GRAVADA</Text>
              <Text style={{ fontSize: 7 }}>{Number(op_gravada).toFixed(2)}</Text>
            </View>
          )}
          ...
          <View style={styles_ticket.total}>
            ...
            <Text style={{ fontSize: 7, fontWeight: 'bold' }}>{Number(total).toFixed(2)}</Text>
          </View>
          
          {totalConLetras && (
            <Text style={{ fontSize: 7, marginTop: 4 }}>{NumeroALetras(total)}</Text>
          )}
        </View>
