...
export default function BottomNavUI({ className }: { className?: string }) {
  ...
  return (
    <div className={`px-3 py-4 sm:px-4 sm:py-6 md:px-6 md:py-8 lg:px-8 lg:py-10
      ...
      {/* Botón 1: Gestión Comercial - Solo si tiene permiso */}
      {can(permissions.GESTION_COMERCIAL_E_INVENTARIO_INDEX) && (
        <ButtonBase
          size='lg'
          color='success'
      ...
      {/* Botón 2: Facturación - Solo si tiene permiso */}
      {can(permissions.FACTURACION_ELECTRONICA_INDEX) && (
        <ButtonBase
          size='lg'
          color='warning'
          className='w-[calc(50%-0.25rem)] sm:w-[calc(50%-0.375rem)]
      ...
      {/* Botón 3: Gestión Contable - Solo si tiene permiso */}
      {can(permissions.GESTION_CONTABLE_Y_FINANCIERA_INDEX) && (
        <ButtonBase
          size='lg'
          color='danger'
          className='w-[calc(50%-0.25rem)] sm:w-[calc(50%-0.375rem)]
      ...
      {/* Botón 4: Reportes - Solo si tiene permiso */}
      {can(permissions.REPORTES_INDEX) && (
        <ButtonBase
          size='lg'
          color='info'
          className='w-[calc(50%-0.25rem)] sm:w-[calc(50%-0.375rem)]
      ...
      {/* Botón 5: Configuración - Solo si tiene permiso */}
      {can(permissions.CONFIGURACION_INDEX) && (
        <ButtonBase
          size='lg'
          color='info'
          className='w-[calc(50%-0.25rem)] sm:w-[calc(50%-0.375rem)]
                     ...
                     text-xs sm:text-sm md:text-base lg:text-lg xl:text-xl
                     min-h-[5rem] sm:min-h-[6rem] md:min-h-[7rem] lg:min-h-[9rem] xl:min-h-[10rem]'
          onClick={() => router.push('/ui/configuracion')}
        >
          <MdSettings className='text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl shrink-0' />
          <span className='text-center sm:text-left leading-tight'>Configuración</span>
        </ButtonBase>
      )}
    </div>
  )
