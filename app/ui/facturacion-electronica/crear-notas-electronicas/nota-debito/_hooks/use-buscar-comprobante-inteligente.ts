...
import { useStoreBuscarComprobanteDebito } from '../_store/store-buscar-comprobante'

/**
 * Helper compartido: aplica un comprobante electrónico al form de nota de débito.
 ...
 */
export default function useBuscarComprobanteInteligente(form: FormInstance<FormCreateNotaDebito>) {
  ...
  const modalOpen = useStoreBuscarComprobanteDebito((s) => s.open)
  const setModalOpen = useStoreBuscarComprobanteDebito((s) => s.setOpen)
      ...
      try {
        ...
        if (!comprobante.venta_id) {
          notification.error({
            ...
            description: `El comprobante ${comprobante.serie}-${comprobante.numero} no tiene una venta asociada. Solo se pueden crear notas de débito para comprobantes con venta asociada.`,
            duration: 6,
          })
          return
        }
        ...
        if (!['ACEPTADO', 'ACEPTADO_CON_OBSERVACIONES'].includes(comprobante.estado_sunat)) {
          notification.error({
            ...
            description: `El comprobante ${comprobante.serie}-${comprobante.numero} está en estado "${comprobante.estado_sunat}". Solo se pueden crear notas de débito sobre facturas o boletas ACEPTADAS por SUNAT.`,
            duration: 6,
          })
          return
        }
