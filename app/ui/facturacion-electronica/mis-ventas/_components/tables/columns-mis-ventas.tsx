...
export function useColumnsMisVentas() {
  const columnDefs: ColDef<getVentaResponseProps>[] = [
    ...
    },
    {
      ...
      // el individual de cada producto (unidades_derivadas) + el de los vales.
      // Se muestra CON IGV, consistente con la columna "Total".
      //
      // Vales: `descuento_aplicado` viene como MONTO en soles para monto fijo /
      // producto gratis / 2x1, pero como TASA (ej. 6 = 6%) para los de
      // PORCENTAJE. Por eso los de % se calculan tasa × base de productos.
      headerName: "Descuentos",
      colId: "descuentos_total",
      field: "productos_por_almacen",
      width: 120,
      valueGetter: (params) => {
        if (params.data)
        ...
        let descuentoProductos = 0;
        let baseProductos = 0; // Σ precio×cantidad (con IGV): base de vales de %.
        for (const producto of productos) {
          for (const unidad of producto.unidades_derivadas || []) {
            const cantidad = Number(unidad.cantidad || 0);
            const precio = Number(unidad.precio || 0); // CON IGV
            const recargo = Number(unidad.recargo || 0);
            const descuento = Number(unidad.descuento || 0);
            const subtotalConRecargo = precio * cantidad + recargo;
            const montoDescuento =
        ...
        const vales = data?.vales_aplicados || [];
        let descuentoVales = 0;
        for (const vale of vales) {
          const valor = Number(vale.descuento_aplicado || 0);
          descuentoVales +=
        ...
        return descuentoProductos + descuentoVales;
      },
      valueFormatter: (params) => {
