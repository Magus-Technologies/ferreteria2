...
export default function ModalCotizaciones({
    ...
    },
    {
      ...
      valueGetter: (params) => {
        ...
        return cotizacion.productos_por_almacen.reduce((sum, pa) => {
          const subtotalProducto = (pa.unidades_derivadas || []).reduce(
            (subSum, ud) => {
  ...
  const handleDoubleClick = (data: Cotizacion | undefined) => {
    ...
    setOpen(false);
    router.push(
      `/ui/facturacion-electronica/mis-ventas/crear-venta?cotizacion=${data.id}`,
    );
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <ButtonBase color="info" size="md" className="flex items-center gap-2">
          <FaSearch /> Buscar
        </ButtonBase>
      </div>
