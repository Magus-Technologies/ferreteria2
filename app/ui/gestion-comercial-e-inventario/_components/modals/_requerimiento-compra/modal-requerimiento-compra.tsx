...
import StepProductos from "../_requerimiento-interno/step-productos"
...
interface ModalRequerimientoCompraProps {
    open: boolean
    onClose: () => void
...
export default function ModalRequerimientoCompra({
    open,
    onClose,
    productosDisponibles = [],
    tipoSolicitud = 'OC',
}: ModalRequerimientoCompraProps) {
    const [currentStep, setCurrentStep] = useState(0)

    const formHook = useRequerimientoForm(tipoSolicitud)
    ...
    const esSOC = tipoSolicitud === 'SOC'
    const modalTitle = esSOC ? 'Nueva Solicitud de Orden de Compra' : 'Nueva Orden de Compra'
    const successDocLabel = esSOC ? 'Solicitud de Orden de Compra' : 'Orden de Compra'
                    ...
                    {submitHook.isSubmitted ? (
                        <div className="flex flex-col items-center justify-center py-12 text-center">
                            ...
                            <p className="text-sm text-slate-600 max-w-sm leading-relaxed mb-4">
                                El requerimiento ha sido enviado al área de supervisión. Se generará la {successDocLabel} correspondiente una vez aprobado.
                            </p>
                            {submitHook.requerimientoCreado && (
