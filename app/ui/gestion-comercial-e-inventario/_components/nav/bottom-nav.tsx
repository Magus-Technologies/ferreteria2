...
import { FaTruck, FaTruckLoading, FaWrench, FaClipboardList } from "react-icons/fa";
import ButtonNav from "~/app/_components/nav/button-nav";
import { useState, useRef, useEffect, useCallback, createElement } from "react";
import { Drawer, Dropdown } from "antd";
import { HiMenuAlt3, HiDotsHorizontal } from "react-icons/hi";
...
export default function BottomNav({ className }: { className?: string }) {
  ...
  const overflowMenuItems = overflowItems.map((item) => {
    ...
    return {
      key: item.id,
      label: (
        <span className={isActive ? 'font-bold text-emerald-600' : ''}>
          {item.label}
        </span>
      ),
      icon: Icon ? createElement(Icon, { className: isActive ? 'text-emerald-600' : '' } as any) : null,
