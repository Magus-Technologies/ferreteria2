...
export default function GananciasPorRecomendacion() {
  const filtros = useFiltrosDashboardGCF()

  const { data } = useQuery({
    queryKey: ['contable-ganancias-recomendacion', filtros],
    queryFn: async () => {
      const res = await dashboardContableApi.gananciasPorRecomendacion(filtros)
  ...
  if (chartData.length === 0) {
    return (
      <div className='flex items-center justify-center h-40 text-gray-400 text-sm'>
        Sin ventas por recomendación en el periodo
      </div>
    )
  }
