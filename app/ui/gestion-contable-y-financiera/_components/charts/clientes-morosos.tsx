...
export default function ClientesMorosos() {
  const filtros = useFiltrosDashboardGCF()

  const { data } = useQuery({
    queryKey: ['contable-clientes-morosos', filtros],
    queryFn: async () => {
      const res = await dashboardContableApi.clientesMorosos(filtros)
