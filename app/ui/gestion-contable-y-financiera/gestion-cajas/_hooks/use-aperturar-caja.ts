...
import { AperturarCajaFormValues } from '../_components/modal-aperturar-caja'
...
export default function useAperturarCaja({
  ...
  async function crearAperturarCaja(
    ...
    try {
      ...
      const montoTotal = values.vendedores.reduce((sum: number, v: VendedorAsignacion) => sum + v.monto, 0)
